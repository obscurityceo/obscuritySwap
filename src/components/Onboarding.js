...
const RINKEBY_TESTNET_PARAMS = {
  chainId: '0x4',
  chainName: 'Rinkeby Testnet C-Chain',
  nativeCurrency: {
    name: 'Rinkeby',
    symbol: 'ETH',
    decimals: 18
...
const RINKEBY_NETWORK_PARAMS = RINKEBY_TESTNET_PARAMS

// Check if the chain Id is the selected Avalanche chain id
const isRinkebyChain = (chainId) => (
  chainId &&
  chainId.toLowerCase() === RINKEBY_NETWORK_PARAMS.chainId.toLowerCase()
)
...
export class OnboardingButton extends React.Component {
  constructor (props) {
    ...
    this.switchToRinkebyChain = this.switchToRinkebyChain.bind(this)
  }
  ...
  render () {
    ...
    if (!MetaMaskOnboarding.isMetaMaskInstalled()) {
    ...
    } else if (!isRinkebyChain(this.state.chainId)) {
      ...
      return (
        <div>
          ...
          <div>To run this dApp you need to switch to the {RINKEBY_NETWORK_PARAMS.chainName} chain</div>
          <button onClick={this.switchToRinkebyChain}>
            Switch to the {RINKEBY_NETWORK_PARAMS.chainName} chain
          </button>
        </div>
      )
    } else {
